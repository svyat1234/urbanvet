...
export default function About() {
  const { image, imageAlt, heading, description, cards } = HOME_PAGE.about;
  const [cardColors, setCardColors] = useState<string[]>(new Array(cards.length).fill(''));
        ...
        <div className="flex gap-[3.125rem] mt-[5.25rem]">
          {cards.map((card, index) => (
            ...
            >
              ...
              />
              <h3 className="text-[2.5rem] font-bold font-[Circe] leading-[100%]">{card.title}</h3>
              <a href="" className="text-[0.9375rem] mt-[1.75rem] max-w-[240px] leading-[115%]">
                {card.text}
              </a>
              {card.phone ? (
                <a
                ...
                <CtaButton href={card.cta.href} label={card.cta.label} className="mt-[2.3125rem]" />
              ) : null}
            </div>
          ))}
        </div>

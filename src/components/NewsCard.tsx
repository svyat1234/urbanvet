...
export default function NewsCard({ data, className }: NewsCardProps) {
  ...
  return (
    ...
    <Link
      href={`/news/${data.slug}`}
      className={`${styles.card} flex justify-between bg-white transition-colors duration-300 min-h-110 flex-col gap-7.5 self-stretch
      max-lg:min-h-auto
      ${className ?? ''}`.trim()}
    ...
    >
      ...
      <span className={`flex flex-col flex-1 justify-between px-7.5 gap-7.5 pb-7.5
        ...
        max-lg:px-3
        `}>
        <h3 className={`font-[Circe] font-bold leading-[80%] text-[2.5rem] min-h-[62px]
          max-2xl:text-[2rem]
          max-xl:text-[1.7rem]
          max-lg:text-[1.5rem]
          max-md:min-h-auto
        ...
        <span className={`flex items-center justify-between`}>
          <span className={`text-[0.875rem] font-normal bg-[#1D1D1D] text-white h-[42px] flex items-center px-[30px] rounded-[100px] leading-[70%]
            max-sm:px-[20px] max-sm:text-[0.875rem] max-sm:h-[30px]
            `}>
            {publishedLabel}
          </span>
          <div className={`${styles.arrow} max-w-[38px] max-h-[38px] w-[38px] h-[38px]
          max-sm:max-w-[30px] max-sm:max-h-[30px] max-sm:w-[30px] max-sm:h-[30px]
          `}>
